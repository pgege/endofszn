...
export interface StoreNavigation02Props {
...
function CategoryImageGrid({ category }: { category: EcommerceNavCategory }) {
  return (
...
export function StoreNavigation02({
  navigation,
  logo,
  cartItemCount = 0,
  onCartClick,
                ...
                <div className="max-h-[70vh] overflow-y-auto p-4">
                  {navigation.categories.map((category) => (
                    <div key={category.id} className="py-4">
                      ...
                      {category.sections.map((section) => (
                        <ul key={section.id} className="mt-2 space-y-1">
                          {section.items.map((item) => (
                            <li key={item.name}>
                              <a
                                href={item.href}
        ...
        <div className="hidden items-center gap-1 lg:flex">
          {navigation.categories.map((category) => (
            <Popover key={category.id}>
              ...
              >
                <CategoryImageGrid category={category} />
              </PopoverContent>
            </Popover>
          ))}
          {navigation.pages.map((page) => (
