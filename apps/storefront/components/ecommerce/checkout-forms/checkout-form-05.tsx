...
export interface CheckoutForm05Props {
...
export function CheckoutForm05({
  items,
  subtotal = "$99.00",
  shipping = "$5.00",
  tax = "$8.32",
  ...
  return (
    <div
      ...
      className={cn("grid w-full lg:grid-cols-[1fr,400px]", className)}
    >
      <form
        onSubmit={(e) => {
          e.preventDefault()
        ...
        className="border-border bg-background p-6 lg:p-12"
      >
      ...
      <div className="border-border bg-muted p-6 lg:p-12">
          ...
          <div className="mt-4 space-y-4">
            {items.map((item) => (
              <div key={item.id} className="flex gap-4">
                <div className="size-16 shrink-0 overflow-hidden rounded-md bg-background">
                  <img
                    src={item.imageSrc}
                    alt={item.imageAlt}
